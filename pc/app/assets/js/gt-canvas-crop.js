...
ZmCanvasCrop.prototype = {
	...
	readFile: function(){
		...
		if(typeof FileReader==='undefined'){ 
		  ...
		  this._input.addEventListener('change', IEreadFile, false);
		...
		function IEreadFile() {
          ...
          //console.log(getBase64Image(reallocalpath));
      ...
      //       }
       ...
       self.drawCavDown(reallocalpath); 		
	...
	drawCavDown: function(src){
	    ...
	    self._img.onload = function(){
	    	...
	    	if(!self.imgTrue()){
	    		...
	    		$('.rob-alert-cover, .rob-onchangecrop').hide(0,alertError());
...
/*					$('.resize-point').off('mousedown');
	...
	save: function(){
		...
		var self = this;
		var $result = $("<canvas style='display:none' width='"+ self._save.width +"' height='"+ self._save.height +"'></canvas>");
		$('body').append($result);
